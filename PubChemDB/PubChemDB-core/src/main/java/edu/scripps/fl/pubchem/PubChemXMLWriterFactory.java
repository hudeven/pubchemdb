...
public class PubChemXMLWriterFactory {
	...
	public static String assayDesc = "/PC-AssayContainer/PC-AssaySubmit/PC-AssaySubmit_assay/PC-AssaySubmit_assay_descr/PC-AssayDescription",
			...
			assayResults = "PC-AssayDescription_results",
	...
	private String getXML(Node node) throws IOException {
	...
	/**
	 * This method builds a PubChem XML Document using a PCAssay database backed object.
	 * @param assay
	 * @return
	 * @throws Exception
	 */
	public Document build(PCAssay assay) throws Exception {
		InputStream is = getClass().getResource("/edu/scripps/fl/pubchem/aid_template.xml").openStream();
	...
	protected void buildAssayDescription(Document document, PCAssay assay) throws Exception {
		Node node;
		Element element;
		Element root = (Element) document.selectSingleNode(assayDesc);
		...
		attributeAndTextAdd("PC-AssayDescription_activity-outcome-method", assay.getActivityOutcomeMethodId(), assay
				.getActivityOutcomeMethod(), root);
			...
			for (int tt = 0; tt < columns.size(); tt++) {
				...
				if (column.getTID() > 0) {
					...
					if (null != panel) {
						...
						attributeAndTextAdd("PC-AssayPanelTestResult_readout-annot", column.getPanelReadoutTypeId(), column
								.getPanelReadoutType(), element);
					}
					if(DEBUGGING)
	...
	protected  void buildPanels(Document document, PCAssay assay) throws Exception {
	...
	protected void buildTargets(Document document, PCAssay assay) throws Exception {
	...
	protected void buildXRefs(Document document, PCAssay assay) throws Exception {
		List<PCAssayXRef> xrefs = assay.getAssayXRefs();
	...
	protected Document loadPubChemXML(InputStream inputStream) throws Exception {
	...
	// This method puts all of the PC-AssayDescription child elements in the correct order.
	protected void organizeXMLDoc(Document doc) {
		Element parent = (Element) doc.selectSingleNode(assayDesc);
		String[] nodesOrder = { "PC-AssayDescription_description", "PC-AssayDescription_protocol", "PC-AssayDescription_comment", 
				xref, assayResults, "PC-AssayDescription_revision", target, "PC-AssayDescription_activity-outcome-method", 
				"PC-AssayDescription_dr", "PC-AssayDescription_grant-number", "PC-AssayDescription_project-category", "PC-AssayDescription_is-panel", 
				"PC-AssayDescription_panel-info", "PC-AssayDescription_categorized-comment" };
